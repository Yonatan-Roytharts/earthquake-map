...
public class EarthquakeCityMap extends PApplet {
	...
	public void mouseClicked()
	{
		...
		if(lastClicked != null) {
			...
			if(isMarkerClicked(cityMarkers,
							   quakeMarkers,
							   mouseX, 
							   mouseY)) 
			{
				for(Marker quakeMarker : quakeMarkers) {
					double distance = lastClicked.getDistanceTo(quakeMarker.getLocation());
					if(distance <= ((EarthquakeMarker) quakeMarker).threatCircle())
						quakeMarker.setHidden(false);
				}
			}
			else if(isMarkerClicked(quakeMarkers, cityMarkers,  mouseX, mouseY)) {
	...
	private boolean isMarkerClicked(List<Marker> markers, List<Marker> markers2, float x, float y) {
